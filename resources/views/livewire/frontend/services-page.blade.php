<div>
  ...
  <section class="relative overflow-hidden gradient-cyan-soft">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-20 text-center relative">
      ...
      <p class="text-lg text-slate-600 max-w-2xl mx-auto">Professional services tailored to bring your digital ideas to
        life with modern technologies and best practices.</p>
    </div>
  </section>
        ...
        @foreach ($services as $service)
          <div class="glass-card rounded-2xl p-6 sm:p-8 card-hover">
            <div class="flex flex-col sm:flex-row items-start gap-4 sm:gap-5">
              <div class="w-12 h-12 sm:w-14 sm:h-14 gradient-cyan rounded-2xl flex-shrink-0 flex items-center justify-center">
                @switch($service->icon)
                  @case('code')
                ...
                @if ($service->technologies)
                  <div class="flex flex-wrap gap-2">
                    @foreach ($service->technologies as $tech)
                      <span
                        class="px-3 py-1 bg-primary-50 text-primary-600 text-xs font-medium rounded-lg">{{ $tech }}</span>
                    @endforeach
                  </div>
                @endif
              </div>
