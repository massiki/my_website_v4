    ...
    <section class="bg-primary-50/50 border-b border-primary-100/30">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div class="flex items-center gap-2 text-sm">
                <a href="{{ route('projects') }}" class="text-primary-600 hover:text-primary-700 transition-colors">Projects</a>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <div class="flex items-center gap-3 mb-4">
                    <span class="px-3 py-1 bg-primary-50 text-primary-600 text-xs font-semibold rounded-full uppercase tracking-wider">{{ $project->category->name }}</span>
            ...
            <div class="rounded-2xl overflow-hidden mb-8 shadow-lg">
                @if($project->thumbnail)
                    <img src="{{ asset('storage/' . $project->thumbnail) }}" alt="{{ $project->title }}" class="w-full h-64 sm:h-80 object-cover">
                @else
                    <div class="w-full h-64 sm:h-80 bg-gradient-to-br from-primary-100 to-primary-200 flex items-center justify-center">
            ...
            <div class="glass-card rounded-2xl p-6">
                ...
                <div class="flex flex-wrap gap-3">
                    @foreach($project->technologies as $tech)
                        <span class="px-4 py-2 bg-primary-50 text-primary-600 font-medium text-sm rounded-xl">{{ $tech->name }}</span>
                    @endforeach
                </div>
            </div>
            ...
            {{-- Back --}}
            <div class="mt-8">
                <a href="{{ route('projects') }}" class="inline-flex items-center gap-2 text-primary-600 hover:text-primary-700 font-medium transition-colors">
