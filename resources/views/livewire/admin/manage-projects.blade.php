          ...
          <tr class="hover:bg-slate-50/50">
            ...
            <td class="px-4 py-3 text-right space-x-2">
              <button wire:click="edit({{ $project->id }})"
                class="text-primary-600 hover:text-primary-700 font-medium">Edit</button>
              <button wire:click="delete({{ $project->id }})" wire:confirm="Delete this project?"
                class="text-red-500 hover:text-red-700 font-medium">Delete</button>
            </td>
          </tr>
        @empty
          <tr>
            <td colspan="5" class="px-4 py-8 text-center text-slate-500">No projects yet.</td>
          </tr>
        @endforelse
      </tbody>
    </table>
  ...
  @if ($showModal)
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6">
        <h3 class="font-heading font-semibold text-lg text-slate-800 mb-6">{{ $editingId ? 'Edit' : 'Create' }} Project
        </h3>
        <form wire:submit="save" class="space-y-4">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="sm:col-span-2">
              ...
              <input wire:model="title" type="text"
                class="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:border-primary-400 focus:ring-2 focus:ring-primary-100 outline-none text-sm">
              @error('title')
            ...
            <div class="sm:col-span-2">
              ...
              <textarea wire:model="short_description" rows="2"
                class="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:border-primary-400 outline-none text-sm resize-none"></textarea>
            ...
            <div>
              ...
              <input wire:model="github_url" type="url"
                class="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:border-primary-400 outline-none text-sm">
            </div>
            <div class="sm:col-span-2">
              ...
              <div class="flex flex-wrap gap-2">
                @foreach ($technologies as $tech)
                  <label
                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border cursor-pointer text-xs font-medium transition-colors {{ in_array($tech->id, $selectedTechs) ? 'bg-primary-50 border-primary-300 text-primary-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50' }}">
                    <input type="checkbox" wire:model.live="selectedTechs" value="{{ $tech->id }}" class="hidden">
                    {{ $tech->name }}
                  </label>
                @endforeach
              </div>
          ...
          <div class="flex justify-end gap-3 pt-4">
            <button type="button" wire:click="$set('showModal', false)"
              class="px-4 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-100 rounded-xl">Cancel</button>
